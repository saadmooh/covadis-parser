...
function processFile(filepath, label) {
  ...
  const helper = new dxf.Helper(text);
  ...
  // Parse MTEXT: "R1\PCT : 433.05\PCR : 430.75\PP : 2.30"
  ...
  // Match labels to inserts by Y-proximity (threshold 300)
  ...
  // ========== 3. EU 1_Canalisations PIPES (geometry) ==========
  ...
  // Classify each pipe and extract real ones
  const planPipes = [];
  for (const e of allCanalLines) {
    ...
    const anchoredVerts = e.vertices.map(v => {
      ...
      return {
        x: v.x, y: v.y,
        manholeIdx: nearest.d < 2 ? nearest.idx : -1,
        manholeDist: nearest.d < 2 ? nearest.d : undefined
      };
    });

    // For start/end, find nearest manhole (larger threshold)
    const connections = [
  ...
  // ========== 4. PIPE LABELS (EU 1_Canalisations_Habillage) ==========
  ...
  // Enrich manholes with profile data
  ...
  // Apply profile data to manholes
  ...
  return {
    ...
    profileSegments: {
      count: profileSegments.length,
      items: profileSegments
    },
    pipesEU1: {
    ...
    pipeLabels: {
      totalTexts: pipeTexts.length,
      groups: parsedLabels.length,
      items: parsedLabels
    },
    dn200: {
      count: dn200Pipes.length,
      totalLength: Math.round(dn200Pipes.reduce((s,p) => s + p.length, 0) * 100) / 100
    },
    assai: {
      inserts: assaiInserts.length,
      lines: assaiLines.length,
      linesDetail: assaiLines
    },
    reseauProjete: {
      count: reseauProj.length
    },
    newEu1Inserts: {
      count: newEu1Inserts.length
    }
  };
}

// Process all files
const n1 = processFile('network 1.dxf', 'Network 1');
const n12 = processFile('networks 1+2.dxf', 'Networks 1+2 (EU 1 + labels + DN200 + assai)');
const n2 = processFile('network 2.dxf', 'Network 2');

const output = {
  generatedAt: new Date().toISOString(),
  description: 'Extraction complète des données de réseaux d\'assainissement Covadis',
  methodology: `
    1. Les manholes sont extraits des INSERTs sur "EU 1_Regards" (position) et des MTEXTs sur "EU 1_Regards_Habillage" (CT/CR/PP).
    2. Les MTEXTs sont parsés en splittant sur le séparateur \\P (retour à la ligne MTEXT).
    3. Les labels sont matchés aux INSERTs par proximité Y (seuil 300 unités).
    4. Les profils sont extraits des layers "EU 1_PL_*_Textes" avec leurs sections standard (auto-détection par regex).
    5. Les pipes EU 1_Canalisations sont filtrées: élimination des éléments en coordonnées négatives (zone profil) et des lignes de construction (verticales/horizontales/diagonales étroites). Le filtre est adapté automatiquement aux différents systèmes de coordonnées.
    6. Les labels pipes (EU 1_Canalisations_Habillage) sont groupés par proximité (5 unités).
    7. Les profils sont matchés aux pipes par longueur totale.
    8. Les manholes sont enrichis avec les données de profil via interpolation spatiale sur la géométrie de la pipe matchée.
  `.trim(),
  network1: n1,
  networks12: n12,
  network2: n2
...
fs.writeFileSync('sewer_data_extraction.json', JSON.stringify(output, null, 2));
console.log('Written sewer_data_extraction.json (' + JSON.stringify(output).length + ' bytes)');
